    ...
    if os.path.exists(configFile):
        try:
            ...
            if currentConfig.has_section('Main Options'):
                ...
                if currentConfig.has_option('Main Options',
                        'militaryTime'):
                    preferences['militaryTime'] = \
                        int(currentConfig.get('Main Options',
                            'militaryTime'))
                ...
                if currentConfig.has_option('Main Options', 'mood'):
                    preferences['mood'] = \
                        int(currentConfig.get('Main Options', 'mood'))

                # Check for theme

                if currentConfig.has_option('Main Options', 'theme'):
                    preferences['theme'] = \
                        int(currentConfig.get('Main Options', 'theme'))
                ...
                if currentConfig.has_option('Main Options', 'allNight'):
                    preferences['allNight'] = \
                        int(currentConfig.get('Main Options', 'allNight'
                            ))
            ...
            if currentConfig.has_section('Alarms'):
                if currentConfig.has_option('Alarms', 'alarmDict'):
                    ...
                    newAlarmsRaw = currentConfig.get('Alarms',
                            'alarmDict')
            ...
            result = note.system_note_dialog("Can't read config",
                    type='notice')
            print result
...
def savePrefs():
    ...
    if os.path.exists(configFile) == 0:
        ...
        try:
            ...
            osso_c = osso.Context('osso_test_note', '0.0.1', False)
            note = osso.SystemNote(osso_c)
            result = note.system_note_dialog("Can't create config file"
                    , type='notice')
            return False
    ...
    if currentConfig.has_option('Alarms', 'alarmDict') == 0:
        ...
        if pickle.loads(currentConfig.get('Alarms', 'alarmDict')) \
            != alarms:
            ...
            currentConfig.set('Alarms', 'alarmDict',
                              pickle.dumps(alarms))
