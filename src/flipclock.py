...
def main():
	...
	if (ci.cliOptions.fixAlarms == True):
		...
		if (len(broken) > 0):
			dialog = hildon.Note("confirmation", (hildon.Window(), str(len(broken)) + " broken FlipClock alarms were found in AlarmD queue; Do you want to fix these now?", gtk.STOCK_DIALOG_WARNING))
			...
			if (resp == gtk.RESPONSE_OK):
				...
				dialog = hildon.Note("information", (hildon.Window(), str(fixedCount) + " of " + str(len(broken)) + " broken FlipClock alarms were fixed!", gtk.STOCK_DIALOG_INFO))
	...
	gobject.timeout_add((60-int(str(theTime[17])+str(theTime[18])))*1000, ci_eventHandlers.drawClockEvent)
	...
	name = dbus.service.BusName(ci.dbusSettings["interface"], ci.session_bus)
