...
class mediaPlayer:
    ...
    def setupPipe(self, outputModule):
        ...
        if outputModule == 'mp3':
            pipestr = \
                'gnomevfssrc name="source" ! dspmp3sink name="sink"'
        elif outputModule == 'wav':
            pipestr = \
                'gnomevfssrc name="source" ! wavparse ! dsppcmsink name="sink"'
...
class mediaControllerClass:
    ...
    def loadFile(self, fileToPlay):
        if fileToPlay.lower().endswith('mp3'):
            ...
            self.currentPlayer = mediaPlayer('mp3')
            self.fileType = 'mp3'
        ...
        self.currentPlayer.playerPipe.get_bus().add_watch(self.watchStatus)
    ...
    def fadeSoundIn(self, time=30000, maxVol=50):

        # Start by setting sound to 0 volume

        self.currentPlayer.setVolume(0)
