...
def bg_night():
    ci.wi_screen.fill(ci.moods[ci_config.preferences['mood'
                      ]]['nightColor'])
...
def drawclock(screenBuf):
  ...
  # ################## ROB UPDATE ###############
    ...
    if ci_config.preferences['militaryTime'] == 1 \
        or ci_config.preferences['militaryTime'] == 0 \
        and int(ci.timeDigits[0]) > 0:

        # Draw first digit if 24 hour mode or first digit is greater than 0

        screenBuf.blit(numbers[int(ci.timeDigits[0])],
                       coords['ore_decine'])  # ORE    (11ma lettera)

    drawSecondDigit = False
    if ci_config.preferences['militaryTime'] == 1:
        drawSecondDigit = True
    else:
        if int(ci.timeDigits[1]) > 0 or int(ci.timeDigits[1]) == 0 \
            and int(ci.timeDigits[0]) > 0:
            drawSecondDigit = True
    ...
    if drawSecondDigit:
        ...
        screenBuf.blit(numbers[int(ci.timeDigits[1])],
                       coords['ore_unita'])
  ...
  # Draw AM/PM thing........
    ...
    screenBuf.blit(numbers[int(ci.timeDigits[2])],
                   coords['minuti_decine'])  # MINUTI (14ma lettera)
...
def drawnight(screenBuf):
  ...
  # ################## ROB UPDATE ###############
    ...
    if ci_config.preferences['militaryTime'] == 1 \
        or ci_config.preferences['militaryTime'] == 0 \
        and int(ci.timeDigits[0]) > 0:
        ci.pygame.draw.rect(screenBuf,
                            ci.moods[ci_config.preferences['mood'
                            ]]['nightColor'], (ci.ni_coords['ore_decine'
                            ][0], ci.ni_coords['ore_decine'][1], 150,
                            257))

        # Draw first digit if 24 hour mode or first digit is greater than 0

        screenBuf.blit(ci.ni_numeri[int(ci.timeDigits[0])],
    ...
    if drawSecondDigit:
        ...
        ci.pygame.draw.rect(screenBuf,
                            ci.moods[ci_config.preferences['mood'
                            ]]['nightColor'], (ci.ni_coords['ore_unita'
                            ][0], ci.ni_coords['ore_unita'][1], 150,
                            257))
        screenBuf.blit(ci.ni_numeri[int(ci.timeDigits[1])],
                       ci.ni_coords['ore_unita'])

    ci.pygame.draw.rect(screenBuf, ci.moods[ci_config.preferences['mood'
                        ]]['nightColor'], (ci.ni_coords['minuti_decine'
                        ][0], ci.ni_coords['minuti_decine'][1], 150,
                        257))
    screenBuf.blit(ci.ni_numeri[int(ci.timeDigits[2])],
                   ci.ni_coords['minuti_decine'])  # MINUTI (14ma lettera)

    ci.pygame.draw.rect(screenBuf, ci.moods[ci_config.preferences['mood'
                        ]]['nightColor'], (ci.ni_coords['minuti_unita'
                        ][0], ci.ni_coords['minuti_unita'][1], 150,
                        257))
    screenBuf.blit(ci.ni_numeri[int(ci.timeDigits[3])],
...
def drawdate(buffer=None):
  ...
  # Small fix to make sure dates have the proper suffix
    ...
    msg = ci.font.render('%s  %s %s' % (dayn, dday, dateSuf), True,
                         (55, 55, 55))
