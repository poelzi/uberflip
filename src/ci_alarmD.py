...
defaultFlags = alarmd.ALARM_EVENT_NO_DIALOG | alarmd.ALARM_EVENT_BOOT | alarmd.ALARM_EVENT_ACTIVATION | alarmd.ALARM_EVENT_SHOW_ICON
...
def listAlarmIDs(startTime=0, endTime=sys.maxint, flags=defaultFlags):
	theIDs = alarmdDBus.query_event(dbus.UInt64(startTime), dbus.UInt64(endTime), dbus.Int32(flags), dbus.Int32(flags));
...
def listAlarms(filterArr=None, startTime=0, endTime=sys.maxint, flags=defaultFlags):
	print flags
...
def setFlipAlarm(alarmObj):
	...
	action.extend(['arguments', dbus.Array([myAlarm["alarm_time"], alarmObj["alarm_index"]], signature=dbus.Signature('v'))]);
	...
	if (myAlarm["recurrence_count"] != 0):
		event.extend(['recurr_interval', dbus.UInt32(myAlarm["recurrence"])])
		event.extend(['recurr_count', dbus.Int32(myAlarm["recurrence_count"])])
		...
		if (int(thisAlarm["alarm_time"]) > 0):
			if (ci_config.preferences["useAlarmD"] == 1):
				...
				if (returnedCookie < 1):
					...
					ci_config.alarms[alarmIndex]["alarm_cookie"] = int(returnedCookie)
