...
def flipping_elem_all_clock(elem, delay):
  for key in ["ore_decine", "ore_unita", "minuti_decine", "minuti_unita"]:
...
def flipclock():			#Flip according to the clock events
  if ci.fcmode==ci.FCMODE_CLOCK:
    for elem in [ci.cl_numeri[int(ci.orario[15])], ci.s1, ci.s2, ci.s3]:
      flipping_element(elem, ci.cl_coords["minuti_unita"], 25)
      if int(ci.orario[15])==0:
        flipping_element(elem, ci.cl_coords["minuti_decine"], 25)
        if int(ci.orario[14])==0: flipping_element(elem, ci.cl_coords["ore_unita"], 25)