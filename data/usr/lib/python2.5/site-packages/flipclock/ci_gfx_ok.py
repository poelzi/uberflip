...
#************************************* MODES ******************************************************
def drawclock():
  ci.cl_screen.blit(ci.cl_numeri[int(ci.orario[11])], ci.cl_coords["ore_decine"])     #ORE    (11ma lettera)
  ci.cl_screen.blit(ci.cl_numeri[int(ci.orario[12])], ci.cl_coords["ore_unita"])
  ci.cl_screen.blit(ci.cl_numeri[int(ci.orario[14])], ci.cl_coords["minuti_decine"])  #MINUTI (14ma lettera)
  ci.cl_screen.blit(ci.cl_numeri[int(ci.orario[15])], ci.cl_coords["minuti_unita"])
...
def drawsveglia():
  ...
  if ci.fcmode==0:                    #********************CLOCK SCREEN
    ...
    drawswitch(ci.sw_config[0], posx, posy, x, y, posoff, poson)
    posx,posy,x,y,posoff,poson=ci.mt_switch_coords
    drawswitch(ci.sw_config[1], posx, posy, x, y ,posoff, poson)
  elif ci.fcmode==3:                  #****************** WINDO MODE
    str_ala="%d%d:%d%d" %(ci.sveglia[0],ci.sveglia[1],ci.sveglia[2],ci.sveglia[3])    
    posx,posy,x,y,posoff,poson=ci.al_switch_coords
    drawswitch(ci.sw_config[0],posx-80, posy-59, x, y, posoff-80, poson-80)
    posx,posy,x,y,posoff,poson=ci.mt_switch_coords
    drawswitch(ci.sw_config[1],posx-80, posy-59, x, y, posoff-80, poson-80)
    y0=y0-60
...
def drawdate():
  ...
  msg=ci.font.render("%s - %s th"%(dayn,dday),True,(55,55,55))
  ci.cl_screen.blit(msg,(x1,y1))       #mont year

def gfx_refresh():
  ci.pygame.display.flip()

def drawswitch(switch,posx,posy,x,y,posoff,poson):
