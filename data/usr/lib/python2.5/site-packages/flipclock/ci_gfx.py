...
###################### NOT HAPPENING ANYMORE, MOVED BACKGROUND DRAWINGS TO EVENT MODULE ##################
def bg_alarm():
...
def drawclock(screenBuf):
  ...
  if ((ci_config.preferences["militaryTime"] == 1) or (ci_config.preferences["militaryTime"] == 0 and (int(ci.timeDigits[0]) > 0))):
		...
		screenBuf.blit(ci.cl_numeri[int(ci.timeDigits[0])], ci.cl_coords["ore_decine"])     #ORE    (11ma lettera)
  ...
  if (drawSecondDigit):
		#Draw second digit if 24 hour mode or first digit greater than 0
 		screenBuf.blit(ci.cl_numeri[int(ci.timeDigits[1])], ci.cl_coords["ore_unita"])

  #Draw AM/PM thing		
  if (ci_config.preferences["militaryTime"] == 0):
  		if ci.isPM:
  			screenBuf.blit(ci.cl_numeri[17], ci.cl_coords["ore_decine"])     #ORE    (11ma lettera)
  		else :
  			screenBuf.blit(ci.cl_numeri[13], ci.cl_coords["ore_decine"])     #ORE    (11ma lettera)
  
    
  screenBuf.blit(ci.cl_numeri[int(ci.timeDigits[2])], ci.cl_coords["minuti_decine"])  #MINUTI (14ma lettera)
  screenBuf.blit(ci.cl_numeri[int(ci.timeDigits[3])], ci.cl_coords["minuti_unita"])
